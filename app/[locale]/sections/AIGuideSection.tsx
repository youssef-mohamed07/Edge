        ...
        <div className={`text-center transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          ...
          <form onSubmit={handleSubmit} className="relative max-w-md mx-auto">
            <input
              ...
              className={`w-full px-8 py-4 border-2 border-true-cobalt/20 text-center text-true-cobalt placeholder-true-cobalt/40 focus:outline-none focus:border-royal-azure transition-colors ${isRTL ? "font-[var(--font-cairo)]" : ""}`}
            />
            {inputValue && (
              <button
                type="submit"
                className={`absolute top-1/2 -translate-y-1/2 w-10 h-10 bg-royal-azure text-white flex items-center justify-center hover:bg-true-cobalt transition-colors ${isRTL ? "left-2" : "right-2"}`}
              >
