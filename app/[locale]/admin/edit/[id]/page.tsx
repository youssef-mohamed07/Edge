...
export default function EditPostPage() {
  ...
  const fetchPost = async () => {
    try {
      ...
      setFormData({
        titleEn: data.post.title.en,
        titleAr: data.post.title.ar,
        excerptEn: data.post.excerpt.en,
        excerptAr: data.post.excerpt.ar,
        contentEn: data.post.content.en,
        contentAr: data.post.content.ar,
        categoryEn: data.post.category.en,
        categoryAr: data.post.category.ar,
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 mb-4">{t.notFound}</p>
          <Link href={`/${locale}/admin`} className="text-blue-600 hover:underline font-medium">
            {t.cancel}
          </Link>
        </div>
      </div>
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 sticky top-0 z-30">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-slate-800">{t.editPost}</h1>
          </div>
          <button
            onClick={() => router.push(`/${locale === "ar" ? "en" : "ar"}/admin/edit/${postId}`)}
              ...
              <div className="relative">
                ...
                <div className="absolute top-3 right-3 flex gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 bg-white text-slate-700 text-sm rounded-lg hover:bg-slate-100 transition-all shadow-sm"
                  >
                    {t.changeImage}
                  </button>
                  <button
                    ...
                    className="px-3 py-1.5 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700 transition-all"
                  >
                    {t.removeImage}
                  </button>
                </div>
