              ...
              <ScrollReveal key={client.id} delay={index * 200} direction="up">
                <div 
                  className={`
                    ...
                    ${activeClient === client.id ? 'ring-2 ring-royal-azure shadow-2xl shadow-royal-azure/20' : ''}
                    ${isRTL ? 'text-right' : ''}
                  `}
                  onClick={() => setActiveClient(activeClient === client.id ? null : client.id)}
                >
                  {/* Background Pattern */}
                  <div className="absolute inset-0 bg-gradient-to-br from-royal-azure/5 to-[#60A5FA]/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl" />
                  ...
                  <div className="relative z-10 text-center">
                    <h3 className={`text-xl font-bold text-true-cobalt mb-2 group-hover:text-royal-azure transition-colors duration-300 ${isRTL ? 'font-[var(--font-cairo)]' : ''}`}>
                      {client.name}
                    </h3>
                    
                    <div className={`flex items-center justify-center gap-2 mb-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                      <div className="w-4 h-4 rounded-full bg-royal-azure/20 flex items-center justify-center">
                        <div className="w-2 h-2 rounded-full bg-royal-azure" />
                      </div>
                      <span className={`text-sm text-true-cobalt/60 ${isRTL ? 'font-[var(--font-cairo)]' : ''}`}>
                        {isRTL ? client.countryAr : client.country}
                      </span>
                    </div>

                    <p className={`text-sm text-royal-azure font-medium mb-4 ${isRTL ? 'font-[var(--font-cairo)]' : ''}`}>
                  ...
                  {/* Hover Effect Border */}
                  <div className="absolute inset-0 rounded-2xl border-2 border-transparent group-hover:border-royal-azure/20 transition-all duration-500" />
                </div>
              </ScrollReveal>
            ))}
          </div>
            ...
            {Array.from({ length: Math.ceil(clients.length / 3) }).map((_, index) => (
              <button
                ...
                className={`
                  ...
                  ${currentSlide === index 
                    ? 'bg-royal-azure scale-125' 
                    : 'bg-royal-azure/30 hover:bg-royal-azure/50'
                  }
                `}
              />
            ))}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 mt-16">
            {[
              ...
              <div key={index} className="text-center group">
                <div className="text-3xl lg:text-4xl font-bold text-true-cobalt mb-2 group-hover:text-royal-azure transition-colors duration-300">
                  {stat.number}
                </div>
